...
RUN_ROOT="${RUN_ROOT:-emotion_et_prediction/runs/zero_scaling_ablation}"
...
for variant in preserve_zero_rows scale_zero_rows; do
  ...
  echo "Running zero-scaling variant=$variant -> $output_dir"
...
echo "Saved zero-scaling ablation summary to $SUMMARY_PATH"
