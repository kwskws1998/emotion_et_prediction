...
EXP_ROOT="${EXP_ROOT:-emotion_et_prediction/runs/repro_main_$(date +%Y%m%d_%H%M%S)}"
...
COMMON_ENV=(
  ...
  "REPRO_MAX_LENGTH=$REPRO_MAX_LENGTH"
  "DEVICE=$DEVICE"
  "SEED=$SEED"
...
if [[ "$RUN_LR_GRID" == "1" ]]; then
  run_logged repro_iitb_lr_grid \
    env "${COMMON_ENV[@]}" LR_GRID="$LR_GRID" RUN_ROOT="$EXP_ROOT/lr_grid" \
    bash "$SCRIPT_DIR/run_repro_iitb_lr_grid.sh"
fi
...
if [[ "$RUN_ZERO_ABLATION" == "1" ]]; then
  run_logged repro_iitb_zero_scaling_ablation \
    env "${COMMON_ENV[@]}" LR="$ZERO_ABLATION_LR" RUN_ROOT="$EXP_ROOT/zero_scaling_ablation" \
    bash "$SCRIPT_DIR/run_repro_iitb_zero_scaling_ablation.sh"
fi
...
echo "Notebook-compatible main experiments finished. Summary: $EXP_ROOT/final_summary.txt"
